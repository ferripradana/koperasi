...
class ReportNeracaBanding extends Controller
{
    //
    protected $helper;
	...
	public function index(){
    	...
    	return view('admin.reportneracabanding.export', compact('bulan_option','tahun_option'));
    }
    ...
    public function post(Request $request){
    	...
    	$coa_asset = $this->createTree($coa_asset, $bulan_from, $tahun_from, $bulan_to , $tahun_to );

    	$coa_l = COA::where('group_id', array(2) )->get();
    	$coa_l = $this->createTree($coa_l, $bulan_from, $tahun_from, $bulan_to , $tahun_to);

    	$coa_e = COA::where('group_id', array(3) )->get();
    	$coa_e = $this->createTree($coa_e, $bulan_from, $tahun_from, $bulan_to , $tahun_to);
    ...
    private function createTree($coa, $bulan_from, $tahun_from, $bulan_to, $tahun_to){
    	$refs = array();
		$list = array();
