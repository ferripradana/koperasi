...
class ReportNeracaUnit extends Controller
{
    ...
    public function index(){
    	...
    	return view('admin.reportneracaunit.export', compact('bulan_option','tahun_option'));
    }
     ...
     public function post(Request $request){
    	...
    	$coa_asset = $this->createTree($coa_asset, $bulan_from, $tahun_from, $id_unit);

    	$coa_l = COA::where('group_id', array(2) )->get();
    	$coa_l = $this->createTree($coa_l, $bulan_from, $tahun_from, $id_unit);

    	$coa_e = COA::where('group_id', array(3) )->get();
    	$coa_e = $this->createTree($coa_e, $bulan_from, $tahun_from, $id_unit);

    	$coa_in = COA::where('group_id', array(4) )->get();
    	$coa_in = $this->createTree($coa_in, $bulan_from, $tahun_from, $id_unit);

    	$coa_ex = COA::where('group_id', array(5) )->get();
    	$coa_ex = $this->createTree($coa_ex, $bulan_from, $tahun_from, $id_unit);
     ...
     private function createTree($coa, $bulan_from, $tahun_from, $id_unit){
    	$refs = array();
		$list = array();
