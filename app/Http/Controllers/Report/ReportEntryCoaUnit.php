...
class ReportEntryCoaUnit extends Controller
{
	...
	public function index(){
    	...
    	$coa = \App\Model\Acc\COA::select(
                    \DB::raw("CONCAT(code,'-',nama) AS name"),'id','parent_id')
                        ->orderBy('code','asc')
                        ->pluck('name', 'id');
    	return view('admin.reportentrycoaunit.export', compact('bulan_option','tahun_option', 'coa'));
    }
    ...
    public function post(Request $request){
	    ...
	    AND 
	    	h.tanggal >= '".$tanggal_from."' AND h.tanggal <= '".$tanggal_to."' AND h.id_unit = ".$id_unit."
	    ORDER by h.id
    	";
    	$result =  \DB::select($q);  
    	...
    	$q_awal = 'Select  SUM(CASE When dc="D" then amount else 0 End ) as debit, 
	        	   ...
	        	   AND h.tanggal < "'.$tanggal_from.'" and h.id_unit = '.$id_unit.'
    	';
    	...
    	return view('admin.pdf.entrycoaunit',compact('result', 'coa', 'from', 'to', 'saldo_awal', 'posisi', 'normal', 'unit' ));
