...
class ReportLabaRugiBanding extends Controller
{
	protected $helper;
	...
	public function index(){
    	...
    	return view('admin.reportlabarugibanding.export', compact('bulan_option','tahun_option'));
    }
     ...
     public function post(Request $request){
        ...
        $profit_from = $result_laba[0]->amount - $result_beban[0]->amount;
        ...
        $q1 = 'select sum(d. amount) as amount
	        	   ...
	        	   join coa c on (d.coa_id = c.id)
	        	   where c.group_id = 4
	    	       and tanggal >= "'.$tahun_to.'-'.$bulan_to.'-01"
	    ...
	    $q2 = 'select sum(d. amount) as amount
	    	       ...
	    	       and tanggal <= "'.$tahun_to.'-'.$bulan_to.'-31" '; 	
	    $result_beban =  \DB::select($q2);      	              
        $profit_to = $result_laba[0]->amount - $result_beban[0]->amount;

       	return view('admin.pdf.reportlabarugibanding',compact('coa', 'bulan_from', 'bulan_to', 'tahun_from', 'tahun_to', 'profit','profit_from', 'profit_to' ));
     ...
     private function createTree($coa, $bulan_from, $bulan_to, $tahun_from, $tahun_to){
		...
		foreach ($coa as $data) {
	        ...
	        $thisref['total_amount_from'] = (float) $result[0]->amount;
	        ...
	        $thisref['total_amount_from'] = (float) $result[0]->amount;
	        $thisref['total_amount_to'] = (float) $result2[0]->amount;


	        if ($thisref['total_amount_from'] == 0) {
	        	$thisref['persen'] = 0;
	        } else {
	        	$thisref['persen'] = (float) ($thisref['total_amount_from']-$thisref['total_amount_to'])/$thisref['total_amount_from'] * 100; 
	        }
